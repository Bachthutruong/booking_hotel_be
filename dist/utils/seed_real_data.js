...
const generateRooms = (hotelId, basePrice, categories) => {
    const rooms = [];
    // Tầng 1-3: Standard (15 phòng)
    for (let i = 101; i <= 105; i++)
        rooms.push(createRoom(hotelId, i, categories[0], basePrice, 1));
    ...
    for (let i = 401; i <= 405; i++)
        rooms.push(createRoom(hotelId, i, categories[1], basePrice * 1.5, 1.2));
    for (let i = 501; i <= 505; i++)
        rooms.push(createRoom(hotelId, i, categories[1], basePrice * 1.5, 1.2));
    for (let i = 601; i <= 605; i++)
        rooms.push(createRoom(hotelId, i, categories[1], basePrice * 1.5, 1.2));
    // Tầng 7-8: Suite (6 phòng)
    for (let i = 701; i <= 703; i++)
        rooms.push(createRoom(hotelId, i, categories[2], basePrice * 2.5, 2));
    for (let i = 801; i <= 803; i++)
        rooms.push(createRoom(hotelId, i, categories[2], basePrice * 2.5, 2));
    // Tầng 9: President (1 phòng)
    rooms.push(createRoom(hotelId, 901, categories[3], basePrice * 5, 4));
    return rooms;
};
const createRoom = (hotelId, roomNum, category, price, sizeMultiplier) => {
    const baseSize = 30;
...
const importData = async () => {
    ...
    try {
        ...
        await models_1.RoomCategory.deleteMany({});
        await models_1.Review.deleteMany({});
        await models_1.Booking.deleteMany({}); // Cần xóa booking vì liên kết với phòng cũ
        // 2. Tạo Categories
        console.log('📦 Đang tạo danh mục phòng...');
        const createdCategories = await models_1.RoomCategory.insertMany(categoriesData);
        ...
        for (const hData of hotelsData) {
            ...
            const hotel = await models_1.Hotel.create({
                ...hotelInfo,
...
importData();
//# sourceMappingURL=seed_real_data.js.map